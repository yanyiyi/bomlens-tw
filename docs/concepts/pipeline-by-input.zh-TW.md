...
無論採用何種輸入，SBOM 都會依照相同的固定順序進行處理。首先執行正規化，以確保後續各步驟的輸入一致；最後執行簽章，以涵蓋最終產生的 SBOM。虛線表示選用或僅適用於特定輸入的步驟。除簽章與上傳外，各步驟均採盡力而為 (best-effort) 的方式執行；若執行失敗，會發出警告並略過該步驟，而不會中止整個掃描程序。
